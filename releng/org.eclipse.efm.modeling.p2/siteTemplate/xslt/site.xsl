<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/site">
		<table cellspacing="2" cellpadding="0" border="0">
			<xsl:if test="count(feature[contains(@id,'efm')])>0">
				...
				<!-- efm features -->
				<xsl:for-each select="feature[contains(@id,'efm')]">
					<xsl:sort select="@id" />
			...
			<!-- xsl:if test="count(feature[not(contains(@id,'efm'))])>0">
				<tr style="background-color:#DDDDDD">
					<th class="column-header">Feature</th>
					<th class="column-header">Version</th>
					<th class="column-header">Feature Categor(ies)</th>
				</tr>
				<xsl:for-each select="feature[not(contains(@id,'efm'))]">
					<xsl:sort select="@id" />
					<xsl:variable name="rowCol">
						<xsl:if test="position() mod 2 = 1">
							#EEEEEE
